...
static inline unsigned char clif_bl_type(struct block_list *bl, bool walking)
{
	...
	{
	case BL_PC:
		return (disguised(bl) && !pcdb_checkid(status_get_viewdata(bl)->class_)) ? 0x1 : 0x0; // PC_TYPE
	case BL_ITEM:
...
void clif_parse_guild_castle_teleport_request(int fd, map_session_data *sd)
{
	...
	if (map_getmapflag(sd->bl.m, MF_GVG_CASTLE) || map_getmapflag(sd->bl.m, MF_NOTELEPORT) || map_getmapflag(sd->bl.m, MF_NOWARP))
		return;
...
void clif_updatestatus(map_session_data *sd, int type)
{
	...
	case SP_HP:
		...
		WFIFOL(fd, 4) = sd->battle_status.hp ? sd->battle_status.hp : (sd->class_ & MAPID_UPPERMASK) != MAPID_NOVICE ? 1
																													 : sd->battle_status.max_hp / 2;
		break;
	case SP_SP:
...
void clif_skill_estimation(map_session_data *sd, struct block_list *dst)
{
	...
	WBUFW(buf, 12) = (battle_config.estimation_type & 1 ? status->def : 0) + (battle_config.estimation_type & 2 ? status->def2 : 0);
	WBUFW(buf, 14) = status->race;
	WBUFW(buf, 16) = (battle_config.estimation_type & 1 ? status->mdef : 0) + (battle_config.estimation_type & 2 ? status->mdef2 : 0);
...
void clif_viewequip_ack(map_session_data *sd, map_session_data *tsd)
{
	...
	for (int i = 0; i < EQI_MAX; i++)
	{
		...
		if (k >= 0)
		{
			if (tsd->inventory.u.items_inventory[k].nameid == 0 || tsd->inventory_data[k] == NULL)
			{ // Item doesn't exist
				continue;
			}
			...
			if (!itemdb_isequip2(tsd->inventory_data[k]))
			{ // Is not equippable
				continue;
			}
...
void clif_parse_LoadEndAck(int fd, map_session_data *sd)
{
	...
	if (!sd->state.active || (!sd->state.autotrade && !sd->state.pc_loaded))
	...
	if (sd->pd)
	{
		if (battle_config.pet_no_gvg && mapdata_flag_gvg(mapdata))
...
void clif_parse_WalkToXY(int fd, map_session_data *sd)
{
	...
	if (battle_config.official_cell_stack_limit > 0 && sd->bl.x == x + 1 && sd->bl.y == y && map_count_oncell(sd->bl.m, x, y, BL_CHAR | BL_NPC, 1))
		return;
...
void clif_parse_Emotion(int fd, map_session_data *sd)
{
	...
	if (battle_config.basic_skill_check == 0 || pc_checkskill(sd, NV_BASIC) >= 2 || pc_checkskill(sd, SU_BASIC_SKILL) >= 1)
	{
		...
		if (battle_config.client_reshuffle_dice && emoticon >= ET_DICE1 && emoticon <= ET_DICE6)
		{ // re-roll dice
...
void clif_parse_ActionRequest_sub(map_session_data *sd, int action_type, int target_id, t_tick tick)
{
	...
	case 0x02: // sitdown
		...
		if (sd->sc.count && (sd->sc.getSCE(SC_DANCING) ||
							 (sd->sc.getSCE(SC_GRAVITATION) && sd->sc.getSCE(SC_GRAVITATION)->val3 == BCT_SELF))) // No sitting during these states either.
			break;
...
void clif_parse_NpcClicked(int fd, map_session_data *sd)
{
	...
	case BL_NPC:
		...
		{ // Should only show an error message for non-3rd job skills with a running timer
			clif_msg(sd, WORK_IN_PROGRESS);
			break;
		}
#endif
...
///     3 = check password
/// NOTE: This packet is only available on certain non-kRO clients.
void clif_parse_StoragePassword(int fd, map_session_data *sd)
{ //@TODO
  //	struct s_packet_db* info = &packet_db[RFIFOW(fd,0)];
...
enum e_result_validate_emblem
{ // Used as Result for clif_validate_emblem
...
void clif_parse_FriendsListRemove(int fd, map_session_data *sd)
{
	...
	{ // friend not online -- ask char server to delete from his friendlist
		if (chrif_removefriend(char_id, sd->status.char_id))
...
void clif_parse_Mail_getattach(int fd, map_session_data *sd)
{
	...
	int attachment = packet_id == 0x9f1 ? MAIL_ATT_ZENY : packet_id == 0x9f3 ? MAIL_ATT_ITEM
																			 : MAIL_ATT_NONE;
#endif
...
void clif_parse_Auction_setitem(int fd, map_session_data *sd)
{
	...
	if (!pc_can_give_items(sd) || sd->inventory.u.items_inventory[idx].expire_time ||
		...
		!itemdb_canauction(&sd->inventory.u.items_inventory[idx], pc_get_group_level(sd)))
...
std::string clif_quest_string(std::shared_ptr<s_quest_objective> objective)
{
	...
	{
	case ELE_NEUTRAL:
		ele_name = "Neutral Element";
		break;
	case ELE_WATER:
		ele_name = "Water Element";
		break;
	case ELE_EARTH:
		ele_name = "Earth Element";
		break;
	case ELE_FIRE:
		ele_name = "Fire Element";
		break;
	case ELE_WIND:
		ele_name = "Wind Element";
		break;
	case ELE_POISON:
		ele_name = "Poison Element";
		break;
	case ELE_HOLY:
		ele_name = "Holy Element";
		break;
	case ELE_DARK:
		ele_name = "Shadow Element";
		break;
	case ELE_GHOST:
		ele_name = "Ghost Element";
		break;
	case ELE_UNDEAD:
		ele_name = "Undead Element";
		break;
	default:
...
void clif_ranklist(map_session_data *sd, int16 rankingType)
{
	...
	switch (sd->class_ & MAPID_UPPERMASK)
	{ // mypoint (checking if valid type)
