...
int status_damage(struct block_list *src, struct block_list *target, int64 dhp, int64 dsp, int64 dap, t_tick walkdelay, int flag, uint16 skill_id)
{
	...
	{
	case BL_PC:
		pc_damage((TBL_PC *)target, src, hp, sp, ap);
		break;
	case BL_MOB:
		mob_damage((TBL_MOB *)target, src, hp);
		break;
	case BL_HOM:
		hom_damage((TBL_HOM *)target);
		break;
	case BL_MER:
	...
	if (sc && sc->getSCE(SC_KAIZEL) && !map_flag_gvg2(target->m))
	...
	if (target->type == BL_MOB && sc && sc->getSCE(SC_REBIRTH) && !((TBL_MOB *)target)->state.rebirth)
...
int status_heal(struct block_list *bl, int64 hhp, int64 hsp, int64 hap, int flag)
{
	...
	if (hp && sc &&
		...
		status->hp >= status->max_hp / 4) // End auto berserk.
	...
	{
	case BL_PC:
		pc_heal((TBL_PC *)bl, hp, sp, ap, flag);
		break;
	case BL_MOB:
...
int status_revive(struct block_list *bl, unsigned char per_hp, unsigned char per_sp, unsigned char per_ap)
{
	...
	{
	case BL_PC:
		pc_revive((TBL_PC *)bl, hp, sp, ap);
		break;
	case BL_MOB:
		mob_revive((TBL_MOB *)bl, hp);
		break;
	case BL_HOM:
		hom_revive((TBL_HOM *)bl, hp, sp);
		break;
	}
	return 1;
}
...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag)
{
	...
	if (sc && sc->count)
	{
		...
		if (skill_id > 0 && sc->opt1 && sc->opt1 != OPT1_STONEWAIT && sc->opt1 != OPT1_BURNING && skill_id != RK_REFRESH && skill_id != SU_GROOMING && skill_id != SR_GENTLETOUCH_CURE)
		{				   // Stuned/Frozen/etc
		...
		if (sc->getSCE(SC_WINKCHARM) && target && !flag)
...
static unsigned int status_calc_maxhpsp_pc(map_session_data *sd, unsigned int stat, bool isHP)
{
	...
	{ // Calculates MaxHP
		...
		dmax = job->base_hp[level - 1] * (1 + (umax(stat, 1) * 0.01)) * ((sd->class_ & JOBL_UPPER) ? 1.25 : (pc_is_taekwon_ranker(sd)) ? 3
																																	   : 1);
	...
	{ // Calculates MaxSP
		double equip_bonus = 0, item_bonus = 0;
		dmax = job->base_sp[level - 1] * (1 + (umax(stat, 1) * 0.01)) * ((sd->class_ & JOBL_UPPER) ? 1.25 : (pc_is_taekwon_ranker(sd)) ? 3
																																	   : 1);
...
int status_calc_pc_sub(map_session_data *sd, uint8 opt)
{
	...
	if (battle_config.character_size && pc_isriding(sd))
	{ // [Lupus]
	...
	// Zero up structures...
	memset(&sd->hp_loss, 0, sizeof(sd->hp_loss) + sizeof(sd->sp_loss) + sizeof(sd->hp_regen) + sizeof(sd->sp_regen) + sizeof(sd->percent_hp_regen) + sizeof(sd->percent_sp_regen) + sizeof(sd->def_set_race) + sizeof(sd->mdef_set_race) + sizeof(sd->norecover_state_race) + sizeof(sd->hp_vanish_race) + sizeof(sd->sp_vanish_race));
	...
	for (i = 0; i < EQI_MAX; i++)
	{
		...
		if (opt & SCO_FIRST && sd->inventory_data[index]->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(sd->inventory_data[index], sd->bl.m)))
	...
	for (i = 0; i < EQI_MAX; i++)
	{
		...
		if (sd->inventory_data[index])
		{
			...
			{ // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				if (!data)
					continue;
				if (opt & SCO_FIRST && data->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data.get(), sd->bl.m)))
				{ // Execute equip-script on login
	...
	if (sc->count)
	{
		...
		if (sc->getSCE(SC_ARMOR_ELEMENT_WATER))
		...
		if (sc->getSCE(SC_ARMOR_ELEMENT_EARTH))
		{ // This status change should grant card-type elemental resist.
		...
		if (sc->getSCE(SC_ARMOR_ELEMENT_FIRE))
		{ // This status change should grant card-type elemental resist.
		...
		if (sc->getSCE(SC_ARMOR_ELEMENT_WIND))
		{ // This status change should grant card-type elemental resist.
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, status_change *sc)
{
	...
	// No HP or SP regen
	if ((sc->getSCE(SC_POISON) && !sc->getSCE(SC_SLOWPOISON)) || (sc->getSCE(SC_DPOISON) && !sc->getSCE(SC_SLOWPOISON)) || sc->getSCE(SC_BERSERK) || sc->getSCE(SC_TRICKDEAD) || sc->getSCE(SC_BLEEDING) || (sc->getSCE(SC_MAGICMUSHROOM) && sc->getSCE(SC_MAGICMUSHROOM)->val3 == 1) || sc->getSCE(SC_SATURDAYNIGHTFEVER) || sc->getSCE(SC_REBOUND))
		regen->flag = RGN_NONE;
	...
	if (bl->type == BL_ELEM)
	{ // Recovery bonus only applies to the Elementals.
...
#endif
			|| (sc->getSCE(SC_CAMOUFLAGE) && sc->getSCE(SC_CAMOUFLAGE)->val1 < 3) || (sc->getSCE(SC_MAGNETICFIELD) && sc->getSCE(SC_MAGNETICFIELD)->val2 != bl->id) || (sc->getSCE(SC_FEAR) && sc->getSCE(SC_FEAR)->val2 > 0) || (sc->getSCE(SC_HIDING) && (bl->type != BL_PC || (pc_checkskill(BL_CAST(BL_PC, bl), RG_TUNNELDRIVE) <= 0))) || (sc->getSCE(SC_DANCING) && sc->getSCE(SC_DANCING)->val4 && (
#ifndef RENEWAL
																																																																																																			  !sc->getSCE(SC_LONGING) ||
#endif
																																																																																																			  (sc->getSCE(SC_DANCING)->val1 & 0xFFFF) == CG_MOONLIT || (sc->getSCE(SC_DANCING)->val1 & 0xFFFF) == CG_HERMODE)) ||
...
void status_calc_bl_(struct block_list *bl, std::bitset<SCB_MAX> flag, uint8 opt)
{
	...
	{ // Calculate the object's base status too
		...
		case BL_PC:
			...
			break;
		case BL_MOB:
			status_calc_mob_(BL_CAST(BL_MOB, bl), opt);
			break;
		case BL_PET:
			status_calc_pet_(BL_CAST(BL_PET, bl), opt);
			break;
		case BL_HOM:
			status_calc_homunculus_(BL_CAST(BL_HOM, bl), opt);
			break;
		case BL_MER:
			status_calc_mercenary_(BL_CAST(BL_MER, bl), opt);
			break;
		case BL_ELEM:
			status_calc_elemental_(BL_CAST(BL_ELEM, bl), opt);
			break;
		case BL_NPC:
			status_calc_npc_(BL_CAST(BL_NPC, bl), opt);
			break;
		}
	}
...
static unsigned short status_calc_speed(struct block_list *bl, status_change *sc, int speed)
{
	...
	{ // Mado speed is not affected by other statuses
		int val = 0;
...
const char *status_get_name(struct block_list *bl)
{
	...
	case BL_MER:
		return ((TBL_MER *)bl)->db->name.c_str(); // They only have database names which are global, not specific to GID.
	case BL_NPC:
		return ((TBL_NPC *)bl)->name;
	case BL_ELEM:
		return ((TBL_ELEM *)bl)->db->name.c_str(); // They only have database names which are global, not specific to GID.
	}
	return "Unknown";
}
...
struct status_data *status_get_status_data(struct block_list *bl)
{
	...
	case BL_NPC:
		return ((mobdb_checkid(((TBL_NPC *)bl)->class_) == 0) ? &((TBL_NPC *)bl)->status : &dummy_status);
	default:
		return &dummy_status;
	}
}
...
struct status_data *status_get_base_status(struct block_list *bl)
{
	...
	case BL_NPC:
		return ((mobdb_checkid(((TBL_NPC *)bl)->class_) == 0) ? &((TBL_NPC *)bl)->status : NULL);
	default:
		return NULL;
	}
}
...
int status_change_start(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4, t_tick duration, unsigned char flag, int32 delay)
{
	...
	case SC_MADNESSCANCEL:
		if (sc->getSCE(type))
		{ // Toggle the status but still consume requirements.
			status_change_end(bl, type);
			return 0;
		}
		break;
...
int status_change_end(struct block_list *bl, enum sc_type type, int tid)
{
	...
	case SC_CLOSECONFINE:
		if (sce->val2 > 0)
		{
			...
			int range = 1 + skill_get_range2(bl, scdb->skill_id, sce->val1, true) + skill_get_range2(bl, TF_BACKSLIDING, 1, true); // Since most people use this to escape the hold....
...
TIMER_FUNC(status_change_timer)
{
	...
	case SC_PROVOKE:
		if (sce->val4)
		{ // Auto-provoke (it is ended in status_heal)
	...
	case SC_MAGICMUSHROOM:
		if (sce->val4 >= 0)
		{
			...
			if (!flag)
			{ // Random Skill Cast
				if (magic_mushroom_db.size() > 0 && sd && !pc_issit(sd))
				{ // Can't cast if sit
					...
					switch (skill_get_casttype(mushroom_skill_id))
	...
	case SC_DEEPSLEEP:
		if (--(sce->val4) >= 0)
		{ // Recovers 3% HP/SP every 2 seconds.
	...
	case SC_OVERHEAT_LIMITPOINT:
		...
		{ // Cooling
			if (sce->val2 == 0)
			{ // Flag the overheat limit once it has been met.
...
int status_change_timer_sub(struct block_list *bl, va_list ap)
{
	...
	case SC_RUWACH: // Reveal hidden target and deal little dammages if enemy
		...
		if (tsc && tsc->getSCE(SC__SHADOWFORM) && (sce && sce->val4 > 0 && sce->val4 % 2000 == 0) && // For every 2 seconds do the checking
			rnd() % 100 < 100 - tsc->getSCE(SC__SHADOWFORM)->val1 * 10)
	...
	case SC_SIGHTBLASTER:
			...
			status_check_skilluse(src, bl, WZ_SIGHTBLASTER, 2))
		{
			if (sce)
			{
				...
				if (skill_attack(BF_MAGIC, src, src, bl, WZ_SIGHTBLASTER, sce->val1, tick, 0x1000000) && (!su || !su->group || !skill_get_inf2(su->group->skill_id, INF2_ISTRAP)))
