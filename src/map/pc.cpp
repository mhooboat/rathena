...
bool pc_addfame(map_session_data &sd, int count)
{
	...
	case MAPID_ALCHEMIST:
		ranktype = RANK_ALCHEMIST;
		break;
	case MAPID_TAEKWON:
		ranktype = RANK_TAEKWON;
		break;
	default:
...
int pc_equippoint_sub(map_session_data *sd, struct item_data *id)
{
	...
	{
		if (pc_checkskill(sd, AS_LEFT) > 0 || (sd->class_ & MAPID_UPPERMASK) == MAPID_ASSASSIN || (sd->class_ & MAPID_UPPERMASK) == MAPID_KAGEROUOBORO)
...
bool pc_authok(map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	if (class_ == -1 || !job_db.exists(sd->status.class_))
	{ // Invalid class?
	...
	do
	{
		...
		for (const auto &skillsit : tree->skills)
		{
			...
			if (!battle_config.skillfree)
			{
				...
				if (sd->status.base_level < entry->baselv)
				{ // We need to get the actual class in this case
				...
				if (sd->status.job_level < entry->joblv)
				{ // We need to get the actual class in this case
					int c_ = pc_mapid2jobid(sd->class_, sd->status.sex);
			...
			if (!fail)
			{
				...
				if (!sd->status.skill[sk_idx].lv && ((skill->inf2[INF2_ISQUEST] && !battle_config.quest_skill_learn) ||
...
void pc_updateweightstatus(map_session_data *sd)
{
	...
	old_overweight = (sd->sc.getSCE(SC_WEIGHT90)) ? 2 : (sd->sc.getSCE(SC_WEIGHT50)) ? 1
																					 : 0;
#ifdef RENEWAL
	new_overweight = (pc_is90overweight(sd)) ? 2 : (pc_is70overweight(sd)) ? 1
																		   : 0;
#else
	new_overweight = (pc_is90overweight(sd)) ? 2 : (pc_is50overweight(sd)) ? 1
																		   : 0;
#endif
...
void pc_bonus2(map_session_data *sd, int type, int type2, int val)
{
	...
	case SP_SKILL_HEAL: // bonus2 bSkillHeal,sk,n;
		if (sd->state.lr_flag == 2)
			break;
		if (sd->skillheal.size() == MAX_PC_BONUS)
	...
	case SP_SKILL_HEAL2: // bonus2 bSkillHeal2,sk,n;
		if (sd->state.lr_flag == 2)
			break;
		if (sd->skillheal2.size() == MAX_PC_BONUS)
		{ // Better mention this so the array length can be updated. [Skotlex]
	...
	case SP_ADD_SKILL_BLOW: // bonus2 bAddSkillBlow,sk,n;
		if (sd->state.lr_flag == 2)
			break;
		if (sd->skillblown.size() == MAX_PC_BONUS)
		{ // Better mention this so the array length can be updated. [Skotlex]
...
int pc_useitem(map_session_data *sd, int n)
{
	...
	if (!pc_has_permission(sd, PC_PERM_ITEM_UNCONDITIONAL) && itemdb_isNoEquip(id, sd->bl.m))
	{
		...
		if (battle_config.allow_consume_restricted_item && id->flag.delay_consume > 0)
...
enum e_setpos pc_setpos(map_session_data *sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	if (battle_config.revive_onwarp && pc_isdead(sd))
	...
	{ // Misc map-changing settings
		...
		{
			if (curr_map_instance_id > 0)
			{ // Update instance timer for the map on leave
...
uint64 pc_jobid2mapid(unsigned short b_class)
{
	...
	case JOB_BABY_TAEKWON:
		return MAPID_BABY_TAEKWON;
	case JOB_BABY_GUNSLINGER:
		return MAPID_BABY_GUNSLINGER;
	case JOB_BABY_NINJA:
		return MAPID_BABY_NINJA;
	...
	case JOB_BABY_STAR_GLADIATOR:
		return MAPID_BABY_STAR_GLADIATOR;
	case JOB_BABY_REBELLION:
		return MAPID_BABY_REBELLION;
	...
	case JOB_BABY_SHADOW_CHASER:
		return MAPID_BABY_SHADOW_CHASER;
	...
	case JOB_HYPER_NOVICE:
		return MAPID_HYPER_NOVICE;
	case JOB_DRAGON_KNIGHT:
		return MAPID_DRAGON_KNIGHT;
	case JOB_ARCH_MAGE:
		return MAPID_ARCH_MAGE;
	case JOB_WINDHAWK:
		return MAPID_WINDHAWK;
	case JOB_CARDINAL:
		return MAPID_CARDINAL;
	case JOB_MEISTER:
		return MAPID_MEISTER;
	case JOB_SHADOW_CROSS:
		return MAPID_SHADOW_CROSS;
	case JOB_SKY_EMPEROR:
		return MAPID_SKY_EMPEROR;
	case JOB_NIGHT_WATCH:
		return MAPID_NIGHT_WATCH;
	case JOB_SHINKIRO:
	case JOB_SHIRANUI:
		return MAPID_SHINKIRO_SHIRANUI;
		// 4-2 Jobs
	case JOB_IMPERIAL_GUARD:
		return MAPID_IMPERIAL_GUARD;
	case JOB_ELEMENTAL_MASTER:
		return MAPID_ELEMENTAL_MASTER;
	case JOB_INQUISITOR:
		return MAPID_INQUISITOR;
	case JOB_TROUBADOUR:
	case JOB_TROUVERE:
		return MAPID_TROUBADOURTROUVERE;
	case JOB_BIOLO:
		return MAPID_BIOLO;
	case JOB_ABYSS_CHASER:
		return MAPID_ABYSS_CHASER;
...
int pc_mapid2jobid(uint64 class_, int sex)
{
	...
	case MAPID_GANGSI:
		return JOB_GANGSI;
	case MAPID_OKTOBERFEST:
		return JOB_OKTOBERFEST;
	case MAPID_SUMMER2:
		return JOB_SUMMER2;
		// 2-1 Jobs
	...
	case MAPID_BABY_TAEKWON:
		return JOB_BABY_TAEKWON;
	case MAPID_BABY_GUNSLINGER:
		return JOB_BABY_GUNSLINGER;
	case MAPID_BABY_NINJA:
		return JOB_BABY_NINJA;
	...
	case MAPID_BABY_STAR_GLADIATOR:
		return JOB_BABY_STAR_GLADIATOR;
	case MAPID_BABY_REBELLION:
	...
	case MAPID_BABY_RUNE_KNIGHT:
		return JOB_BABY_RUNE_KNIGHT;
	case MAPID_BABY_WARLOCK:
		return JOB_BABY_WARLOCK;
	case MAPID_BABY_RANGER:
		return JOB_BABY_RANGER;
	case MAPID_BABY_ARCH_BISHOP:
		return JOB_BABY_ARCH_BISHOP;
	case MAPID_BABY_MECHANIC:
		return JOB_BABY_MECHANIC;
	case MAPID_BABY_GUILLOTINE_CROSS:
		return JOB_BABY_GUILLOTINE_CROSS;
	case MAPID_BABY_STAR_EMPEROR:
		return JOB_BABY_STAR_EMPEROR;
		// Baby 3-2 Jobs
	case MAPID_BABY_ROYAL_GUARD:
		return JOB_BABY_ROYAL_GUARD;
	...
	case MAPID_BABY_SHADOW_CHASER:
		return JOB_BABY_SHADOW_CHASER;
	case MAPID_BABY_SOUL_REAPER:
		return JOB_BABY_SOUL_REAPER;
		// Doram Jobs
	case MAPID_SUMMONER:
		return JOB_SUMMONER;
	case MAPID_SPIRIT_HANDLER:
		return JOB_SPIRIT_HANDLER;
		// 4-1 Jobs
	case MAPID_HYPER_NOVICE:
		return JOB_HYPER_NOVICE;
	case MAPID_DRAGON_KNIGHT:
		return JOB_DRAGON_KNIGHT;
	case MAPID_ARCH_MAGE:
		return JOB_ARCH_MAGE;
	case MAPID_WINDHAWK:
		return JOB_WINDHAWK;
	case MAPID_CARDINAL:
		return JOB_CARDINAL;
	case MAPID_MEISTER:
		return JOB_MEISTER;
	case MAPID_SHADOW_CROSS:
		return JOB_SHADOW_CROSS;
	case MAPID_SKY_EMPEROR:
		return JOB_SKY_EMPEROR;
	case MAPID_NIGHT_WATCH:
		return JOB_NIGHT_WATCH;
	case MAPID_SHINKIRO_SHIRANUI:
		return sex ? JOB_SHINKIRO : JOB_SHIRANUI;
		// 4-2 Jobs
	case MAPID_IMPERIAL_GUARD:
		return JOB_IMPERIAL_GUARD;
	case MAPID_ELEMENTAL_MASTER:
		return JOB_ELEMENTAL_MASTER;
	case MAPID_INQUISITOR:
		return JOB_INQUISITOR;
	case MAPID_TROUBADOURTROUVERE:
		return sex ? JOB_TROUBADOUR : JOB_TROUVERE;
	case MAPID_BIOLO:
		return JOB_BIOLO;
	case MAPID_ABYSS_CHASER:
		return JOB_ABYSS_CHASER;
...
int pc_getstat(map_session_data *sd, int type)
{
	...
	case SP_POW:
		return sd->status.pow;
	case SP_STA:
		return sd->status.sta;
	case SP_WIS:
		return sd->status.wis;
	case SP_SPL:
		return sd->status.spl;
	case SP_CON:
		return sd->status.con;
	case SP_CRT:
		return sd->status.crt;
	default:
		return -1;
	}
}
...
bool pc_statusup(map_session_data *sd, int type, int increase)
{
	...
	if (needed_points < 0 || needed_points > sd->status.status_point)
	{ // Sanity check
		clif_statusupack(sd, type, 0, 0);
		return false;
	}
...
int pc_dead(map_session_data *sd, struct block_list *src)
{
	...
	if (sd->shadowform_id)
	{ // if we were target of shadowform
	...
	if (battle_config.death_penalty_type && (sd->class_ & MAPID_UPPERMASK) != MAPID_NOVICE // only novices will receive no penalty
		&& !sd->sc.cant.deathpenalty && !mapdata->getMapFlag(MF_NOEXPPENALTY) && !mapdata_flag_gvg2(mapdata))
	{
		...
		if ((battle_config.death_penalty_maxlv & 1 || !pc_is_maxbaselv(sd)) && base_penalty > 0)
		{
			...
			{
			case 1:
				base_penalty = (t_exp)(pc_nextbaseexp(sd) * (base_penalty / 10000.));
				break;
			case 2:
				base_penalty = (t_exp)(sd->status.base_exp * (base_penalty / 10000.));
				break;
			}
			if (base_penalty)
			{ // recheck after altering to speedup
		...
		if ((battle_config.death_penalty_maxlv & 2 || !pc_is_maxjoblv(sd)) && job_penalty > 0)
		{
			...
			{
			case 1:
				job_penalty = (uint32)(pc_nextjobexp(sd) * (job_penalty / 10000.));
				break;
			case 2:
				job_penalty = (uint32)(sd->status.job_exp * (job_penalty / 10000.));
				break;
			}
			if (job_penalty)
			{
	...
	{ // Moved this outside so it works when PVP isn't enabled and during pk mode [Ancyker]
		for (const auto &it : mapdata->drop_list)
		{
			...
			else if (id > 0)
			{
				for (i = 0; i < MAX_INVENTORY; i++)
				{
					if (sd->inventory.u.items_inventory[i].nameid == id && rnd() % 10000 < per && ((type & NMDT_INVENTORY && !sd->inventory.u.items_inventory[i].equip) || (type & NMDT_EQUIP && sd->inventory.u.items_inventory[i].equip) || type & NMDT_ALL))
					{
						if (sd->inventory.u.items_inventory[i].equip)
...
int64 pc_readparam(map_session_data *sd, int64 type)
{
	...
	case SP_MAXWEIGHT:
		...
		break;
	case SP_BASEEXP:
		val = u64min(sd->status.base_exp, MAX_EXP);
		break;
	case SP_JOBEXP:
		val = u64min(sd->status.job_exp, MAX_EXP);
		break;
	case SP_NEXTBASEEXP:
		val = u64min(pc_nextbaseexp(sd), MAX_EXP);
		break;
	case SP_NEXTJOBEXP:
		val = u64min(pc_nextjobexp(sd), MAX_EXP);
		break;
	case SP_HP:
	...
	case SP_MAXSP:
		...
		break;
	case SP_AP:
		val = sd->battle_status.ap;
		break;
	case SP_MAXAP:
		val = sd->battle_status.max_ap;
		break;
	case SP_STR:
		val = sd->status.str;
		break;
	...
	case SP_SITTING:
		...
		break;
	case SP_CHARMOVE:
		val = sd->status.character_moves;
		break;
	case SP_CHARRENAME:
		val = sd->status.rename;
		break;
	case SP_CHARFONT:
		val = sd->status.font;
		break;
	...
	case SP_ROULETTE_BRONZE:
		val = sd->roulette_point.bronze;
		break;
	case SP_ROULETTE_SILVER:
		val = sd->roulette_point.silver;
		break;
	case SP_ROULETTE_GOLD:
		val = sd->roulette_point.gold;
		break;
	...
	case SP_COOKMASTERY:
		val = sd->cook_mastery;
		break;
	case SP_ACHIEVEMENT_LEVEL:
		val = sd->achievement_data.level;
		break;
	case SP_CRITICAL:
	...
	case SP_BASE_ATK:
		...
		break;
	case SP_DEF1:
		val = sd->battle_status.def;
		break;
	case SP_DEF2:
		val = sd->battle_status.def2;
		break;
	case SP_MDEF1:
		val = sd->battle_status.mdef;
		break;
	case SP_MDEF2:
		val = sd->battle_status.mdef2;
		break;
	case SP_HIT:
		val = sd->battle_status.hit;
		break;
	case SP_FLEE1:
		val = sd->battle_status.flee;
		break;
	case SP_FLEE2:
		val = sd->battle_status.flee2;
		break;
	case SP_PATK:
		val = sd->battle_status.patk;
		break;
	case SP_SMATK:
		val = sd->battle_status.smatk;
		break;
	case SP_RES:
		val = sd->battle_status.res;
		break;
	case SP_MRES:
		val = sd->battle_status.mres;
		break;
	case SP_HPLUS:
		val = sd->battle_status.hplus;
		break;
	case SP_CRATE:
		val = sd->battle_status.crate;
		break;
	case SP_DEFELE:
		val = sd->battle_status.def_ele;
		break;
	case SP_MAXHPRATE:
		val = sd->hprate;
		break;
	...
	case SP_SP_RECOV_RATE:
		...
		break;
	case SP_CRITICAL_DEF:
		val = sd->bonus.critical_def;
		break;
	case SP_NEAR_ATK_DEF:
		val = sd->bonus.near_attack_def_rate;
		break;
	case SP_LONG_ATK_DEF:
		val = sd->bonus.long_attack_def_rate;
		break;
	case SP_DOUBLE_RATE:
		val = sd->bonus.double_rate;
		break;
	case SP_DOUBLE_ADD_RATE:
		val = sd->bonus.double_add_rate;
		break;
	case SP_MATK_RATE:
		val = sd->matk_rate;
		break;
	...
	case SP_MAGIC_ATK_DEF:
		...
		break;
	case SP_MISC_ATK_DEF:
		val = sd->bonus.misc_def_rate;
		break;
	case SP_PERFECT_HIT_RATE:
		val = sd->bonus.perfect_hit;
		break;
	case SP_PERFECT_HIT_ADD_RATE:
		val = sd->bonus.perfect_hit_add;
		break;
	case SP_CRITICAL_RATE:
		val = sd->critical_rate;
		break;
	case SP_HIT_RATE:
		val = sd->hit_rate;
		break;
	...
	case SP_CRATE_RATE:
		...
		break;
	case SP_RESTART_FULL_RECOVER:
		val = sd->special_state.restart_full_recover ? 1 : 0;
		break;
	case SP_NO_CASTCANCEL:
		val = sd->special_state.no_castcancel ? 1 : 0;
		break;
	case SP_NO_CASTCANCEL2:
		val = sd->special_state.no_castcancel2 ? 1 : 0;
		break;
	case SP_NO_SIZEFIX:
		val = sd->special_state.no_sizefix ? 1 : 0;
		break;
	case SP_NO_MAGIC_DAMAGE:
		val = sd->special_state.no_magic_damage;
		break;
	...
	case SP_NO_MISC_DAMAGE:
		...
		break;
	case SP_NO_GEMSTONE:
		val = sd->special_state.no_gemstone ? 1 : 0;
		break;
	case SP_INTRAVISION:
		val = sd->special_state.intravision ? 1 : 0;
		break;
	case SP_NO_KNOCKBACK:
		val = sd->special_state.no_knockback ? 1 : 0;
		break;
	case SP_NO_MADO_FUEL:
		val = sd->special_state.no_mado_fuel ? 1 : 0;
		break;
	case SP_NO_WALK_DELAY:
		val = sd->special_state.no_walk_delay ? 1 : 0;
		break;
	case SP_SPLASH_RANGE:
		val = sd->bonus.splash_range;
		break;
	case SP_SPLASH_ADD_RANGE:
		val = sd->bonus.splash_add_range;
		break;
	case SP_SHORT_WEAPON_DAMAGE_RETURN:
		val = sd->bonus.short_weapon_damage_return;
		break;
	case SP_LONG_WEAPON_DAMAGE_RETURN:
		val = sd->bonus.long_weapon_damage_return;
		break;
	case SP_MAGIC_DAMAGE_RETURN:
		val = sd->bonus.magic_damage_return;
		break;
	case SP_REDUCE_DAMAGE_RETURN:
		val = sd->bonus.reduce_damage_return;
		break;
	case SP_PERFECT_HIDE:
		val = sd->special_state.perfect_hiding ? 1 : 0;
		break;
	case SP_UNBREAKABLE:
		val = sd->bonus.unbreakable;
		break;
	case SP_UNBREAKABLE_WEAPON:
		val = (sd->bonus.unbreakable_equip & EQP_WEAPON) ? 1 : 0;
		break;
	case SP_UNBREAKABLE_ARMOR:
		val = (sd->bonus.unbreakable_equip & EQP_ARMOR) ? 1 : 0;
		break;
	case SP_UNBREAKABLE_HELM:
		val = (sd->bonus.unbreakable_equip & EQP_HELM) ? 1 : 0;
		break;
	case SP_UNBREAKABLE_SHIELD:
		val = (sd->bonus.unbreakable_equip & EQP_SHIELD) ? 1 : 0;
		break;
	case SP_UNBREAKABLE_GARMENT:
		val = (sd->bonus.unbreakable_equip & EQP_GARMENT) ? 1 : 0;
		break;
	case SP_UNBREAKABLE_SHOES:
		val = (sd->bonus.unbreakable_equip & EQP_SHOES) ? 1 : 0;
		break;
	case SP_CLASSCHANGE:
		val = sd->bonus.classchange;
		break;
	case SP_SHORT_ATK_RATE:
		val = sd->bonus.short_attack_atk_rate;
		break;
	case SP_LONG_ATK_RATE:
		val = sd->bonus.long_attack_atk_rate;
		break;
	case SP_BREAK_WEAPON_RATE:
		val = sd->bonus.break_weapon_rate;
		break;
	case SP_BREAK_ARMOR_RATE:
		val = sd->bonus.break_armor_rate;
		break;
	case SP_ADD_STEAL_RATE:
		val = sd->bonus.add_steal_rate;
		break;
	case SP_DELAYRATE:
		val = sd->bonus.delayrate;
		break;
	case SP_CRIT_ATK_RATE:
		val = sd->bonus.crit_atk_rate;
		break;
	case SP_UNSTRIPABLE_WEAPON:
		val = (sd->bonus.unstripable_equip & EQP_WEAPON) ? 1 : 0;
		break;
	case SP_UNSTRIPABLE:
	case SP_UNSTRIPABLE_ARMOR:
		...
		break;
	case SP_UNSTRIPABLE_HELM:
		val = (sd->bonus.unstripable_equip & EQP_HELM) ? 1 : 0;
		break;
	case SP_UNSTRIPABLE_SHIELD:
		val = (sd->bonus.unstripable_equip & EQP_SHIELD) ? 1 : 0;
		break;
	case SP_SP_GAIN_VALUE:
		val = sd->bonus.sp_gain_value;
		break;
	case SP_HP_GAIN_VALUE:
		val = sd->bonus.hp_gain_value;
		break;
	case SP_LONG_SP_GAIN_VALUE:
		val = sd->bonus.long_sp_gain_value;
		break;
	case SP_LONG_HP_GAIN_VALUE:
		val = sd->bonus.long_hp_gain_value;
		break;
	case SP_MAGIC_SP_GAIN_VALUE:
		val = sd->bonus.magic_sp_gain_value;
		break;
	case SP_MAGIC_HP_GAIN_VALUE:
		val = sd->bonus.magic_hp_gain_value;
		break;
	case SP_ADD_HEAL_RATE:
		val = sd->bonus.add_heal_rate;
		break;
	case SP_ADD_HEAL2_RATE:
		val = sd->bonus.add_heal2_rate;
		break;
	case SP_ADD_ITEM_HEAL_RATE:
		val = sd->bonus.itemhealrate2;
		break;
	case SP_EMATK:
		val = sd->bonus.ematk;
		break;
	case SP_FIXCASTRATE:
		val = sd->bonus.fixcastrate;
		break;
	case SP_ADD_FIXEDCAST:
		val = sd->bonus.add_fixcast;
		break;
	case SP_ADD_VARIABLECAST:
		val = sd->bonus.add_varcast;
		break;
	case SP_CASTRATE:
	case SP_VARCASTRATE:
#ifdef RENEWAL_CAST
...
#endif
	case SP_CRIT_DEF_RATE:
		val = sd->bonus.crit_def_rate;
		break;
	case SP_ADD_ITEM_SPHEAL_RATE:
		val = sd->bonus.itemsphealrate2;
		break;
	default:
...
bool pc_jobchange(map_session_data *sd, int job, char upper)
{
	...
	if ((b_class & JOBL_UPPER) && !(previous_class & JOBL_UPPER))
	{ // Change from a non t class to a t class -> give points
		sd->status.status_point += battle_config.transcendent_status_points;
		clif_updatestatus(sd, SP_STATUSPOINT);
	}
	else if (!(b_class & JOBL_UPPER) && (previous_class & JOBL_UPPER))
	{ // Change from a t class to a non t class -> remove points
	...
	if ((b_class & JOBL_FOURTH) && !(previous_class & JOBL_FOURTH))
	{ // Change to a 4th job.
		sd->status.trait_point += battle_config.trait_points_job_change;
		clif_updatestatus(sd, SP_TRAITPOINT);
		clif_updatestatus(sd, SP_UPOW);
	...
	else if (!(b_class & JOBL_FOURTH) && (previous_class & JOBL_FOURTH))
	{ // Change to a non 4th job.
		...
		for (j = 0; j < nb_itemCombo; j++)
		{
			...
			for (int16 k = 0; k < EQI_MAX; k++)
			{
				...
				if (itemdb_type(id) != IT_CARD)
				{
					...
					if (j > 0)
					{ // Check if this item not already used
						do_continue = false;
							...
							for (size_t c1 = 0; c1 < nb_itemCombo - 1; c1++)
							{
								if (combo_idx[c1].idx == index && combo_idx[c1].nameid == id)
								{
									for (uint8 c2 = 0; c2 < MAX_SLOTS; c2++)
									{
										...
										{ // Card already recorded (at this same idx)
											do_continue = true;
											break;
										}
									}
...
bool pc_equipitem(map_session_data *sd, short n, int req_pos, bool equipswitch)
{
	...
	if (!(pos & req_pos) || sd->inventory.u.items_inventory[n].equip != 0 || sd->inventory.u.items_inventory[n].attribute == 1)
	{ // [Valaris]
		if (equipswitch)
		{
			clif_equipswitch_add(sd, n, req_pos, ITEM_EQUIP_ACK_FAIL);
	...
	if (pos == EQP_ACC)
	{ // Accessories should only go in one of the two.
		pos = req_pos & EQP_ACC;
		if (pos == EQP_ACC) // User specified both slots.
			pos = (equip_index[EQI_ACC_R] >= 0 && equip_index[EQI_ACC_L] < 0) ? EQP_ACC_L : EQP_ACC_R;

		for (i = 0; i < sd->inventory_data[n]->slots; i++)
	...
	else if (pos == EQP_ARMS && id->equip == EQP_HAND_R)
...
bool pc_unequipitem(map_session_data *sd, int n, int flag)
{
	...
	if (!(flag & 2) && sd->sc.count && (sd->sc.cant.unequip || (sd->sc.getSCE(SC_PYROCLASTIC) && sd->inventory_data[n]->type == IT_WEAPON))) // can't switch weapon
	{
		clif_unequipitemack(sd, n, 0, 0);
		return false;
	}
...
void pc_check_available_item(map_session_data *sd, uint8 type)
{
	...
	if (battle_config.item_check & ITMCHK_INVENTORY && type & ITMCHK_INVENTORY)
	{ // Check for invalid(ated) items in inventory.
		for (i = 0; i < MAX_INVENTORY; i++)
		{
	...
	if (battle_config.item_check & ITMCHK_CART && type & ITMCHK_CART)
	{ // Check for invalid(ated) items in cart.
		for (i = 0; i < MAX_CART; i++)
		{
	...
	if (battle_config.item_check & ITMCHK_STORAGE && type & ITMCHK_STORAGE)
	{ // Check for invalid(ated) items in storage.
	...
	if ((node = sd->bonus_script.head))
	{
		while (node)
		{
			...
			if (strcmpi(script_str, StringBuf_Value(entry->script_buf)) == 0)
			{
				...
				if (flag & BSF_FORCE_REPLACE && entry->tick < newdur)
				{ // Change duration
...
short pc_maxaspd(map_session_data *sd)
{
	...
	return ((sd->class_ & JOBL_THIRD) ? battle_config.max_third_aspd : (((sd->class_ & MAPID_UPPERMASK) == MAPID_KAGEROUOBORO || (sd->class_ & MAPID_UPPERMASK) == MAPID_REBELLION) ? battle_config.max_extended_aspd : ((sd->class_ & MAPID_BASEMASK) == MAPID_SUMMONER) ? battle_config.max_summoner_aspd
